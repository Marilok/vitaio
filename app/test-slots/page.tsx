...
export default function TestSlotsPage() {
  ...
  const fetchSlots = async () => {
    try {
      ...
      const appointmentIds = appointmentTypeIds
        .split(",")
        .map((id) => parseInt(id.trim()))
        .filter((id) => !isNaN(id));
  ...
  return (
    <Container size="md" py="xl">
      <Stack gap="xl">
        <div>
          <Title order={1} mb="sm">
            Testovací stránka API slotů
          </Title>
          <Text c="dimmed">
            Otestujte /api/slots endpoint a vizualizujte dostupné termíny
          </Text>
        </div>
        ...
        {getCurrentAppointment() && (
          <Card withBorder padding="lg" radius="md">
            <Stack gap="md">
              <Text fw={500} size="lg">
                Vyberte datum a čas pro{" "}
                <Text span fw={700}>
                  {getCurrentAppointment() &&
                    (getExaminationTypeDetails(getCurrentAppointment()!)
                      ?.name ||
                      `typu ${getCurrentAppointment()}`)}
                </Text>
              </Text>
                ...
                <Stack gap="sm" style={{ flex: "0 0 250px" }}>
                  ...
                  <Stack gap="xs">
                    {getRequestedAppointments().map((appointmentId, index) => {
                      ...
                      const selectedAppointment = selectedAppointments.find(
                        (apt) => apt.appointmentTypeId === appointmentId
                      );
                          ...
                          <Group gap="sm" align="flex-start">
                            ...
                            <div style={{ flex: 1 }}>
                              <Text
                                ...
                                size="sm"
                                c={
                                  isSelected || isCurrent ? undefined : "dimmed"
                                }
                              ...
                              <Text
                                size="xs"
                                c={
                                  isSelected || isCurrent ? "dimmed" : "dark.3"
                                }
                              >
                  ...
                  {selectedDate && selectedDateSlots.length > 0 && (
                    ...
                    >
                      {selectedDateSlots.map((slot) => (
                        ...
                        >
                          <Text
                            fw={selectedSlot === slot.id ? 600 : 500}
                            size="sm"
                          >
